 ...
 * Run with: npx ts-node --compiler-options '{"module":"CommonJS"}' scripts/test-trend-engine.ts
 */
...
const nfl = loadGamesBySport("NFL");
const ncaaf = loadGamesBySport("NCAAF");
const ncaamb = loadGamesBySport("NCAAMB");
...
console.log(`  Total:  ${(nfl.length + ncaaf.length + ncaamb.length).toLocaleString()} games\n`);
