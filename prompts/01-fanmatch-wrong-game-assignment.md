...
**Fix:** Re-key FanMatch data at fetch time, same pattern as getKenpomRatings(). In `src/lib/kenpom.ts`, modify `getKenpomFanMatch`:
...
Then in pick-engine.ts, the lookup at line ~2497 should use `canonHome`/`canonAway` instead of `game.homeTeam`/`game.awayTeam`:
```typescript
const gameFM = kenpomFanMatch?.find(
...
Same fix needed inside `computeKenPomEdge` (around line 300-305) where FanMatch is looked up using the `homeTeam`/`awayTeam` params — those should be canonical since the function now receives canonical names.
```
